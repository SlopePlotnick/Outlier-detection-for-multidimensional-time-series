...
def create_toy_data(num_of_sequences=10000, sequence_len=50) -> torch.tensor:
    ...
    # 提取特性数据和标签
    train_features = train_df.drop(columns=['timestamp_(min)'], axis=1).values.reshape(-1, 25, 1)  # Shape: 132481 * 25
    # train_features = train_df.drop(columns=['timestamp_(min)']).values  # Shape: 132481 * 25

    # 将数组转化为tensor
    train_data = torch.tensor(train_features, dtype=torch.float)  # Size: 132481 * 25

    test_df = pd.read_csv('test.csv')

    # 提取特性数据和标签
    test_features = test_df.drop(columns=['timestamp_(min)'], axis=1).values.reshape(-1, 25, 1)  # Shape: 132481 * 25
    # test_features = test_df.drop(columns=['timestamp_(min)']).values  # Shape: 132481 * 25

    # 将数组转化为tensor
    test_data = torch.tensor(test_features, dtype=torch.float)  # Size: 132481 * 25
...
def create_dataloaders(batch_size, train_ratio=0.6, val_ratio=0.2):
    ...
    """
    train_data, test_data = create_toy_data()
    l = train_data.shape[0]
    # train_set, val_set = torch.utils.data.random_split(train_set, [110400, 22081])
    all_train_data = train_data
    train_data = all_train_data[:int(l * train_ratio), :]
    val_data = all_train_data[int(train_ratio * l):int(l * (train_ratio + val_ratio)), :]
    test_data = all_train_data[int((train_ratio + val_ratio) * l):, :]
